      ...
      <div class="catalog-groups">
        <section class="catalog-section" aria-labelledby="title-achievements-title">
          ...
          <UTable :data="titleItems" :columns="titleColumns" :loading="loading" empty="暂无记录。" class="admin-table achievement-table">
            ...
            <template #actions-cell="{ row }"><div class="table-actions"><template v-if="isChallengeTitle(row.original)"><UButton :aria-label="editingId === row.original.challengeId ? '收起编辑' : '编辑规则'" icon="i-lucide-pencil" color="neutral" variant="ghost" size="xs" square :loading="isSaving(row.original)" @click="toggleEditing(row.original.challengeId)" /><UPopover v-if="row.original.status !== 'retired'" :open="planningId === row.original.challengeId" @update:open="(open) => { planningId = open ? row.original.challengeId : null; }"><UButton aria-label="计划下线" icon="i-lucide-calendar-clock" color="neutral" variant="ghost" size="xs" square :loading="isSaving(row.original)" /><template #content><UCard class="plan-popover-card"><form class="plan-popover" @submit.prevent="planSunsetting(row.original)"><UFormField label="计划下线版本" required><UInput v-model="retirementVersions[row.original.challengeId]" required placeholder="例如 26.0713.1" :disabled="isSaving(row.original)" /></UFormField><UButton type="submit" label="确认计划" :loading="isSaving(row.original)" :disabled="!retirementVersions[row.original.challengeId]?.trim()" /></form></UCard></template></UPopover><UButton v-if="row.original.status !== 'retired'" aria-label="结束挑战" icon="i-lucide-circle-stop" color="error" variant="ghost" size="xs" square :loading="isSaving(row.original)" @click="openEnd(row.original, $event.currentTarget)" /><UButton v-else aria-label="重新开放" icon="i-lucide-rotate-ccw" color="neutral" variant="ghost" size="xs" square :loading="isSaving(row.original)" @click="reopen(row.original)" /></template><template v-else><UButton v-if="row.original.status === 'active'" aria-label="下线称号" icon="i-lucide-circle-stop" color="error" variant="ghost" size="xs" square :loading="isSaving(row.original)" @click="openEnd(row.original, $event.currentTarget)" /><UButton v-else aria-label="重新开放" icon="i-lucide-rotate-ccw" color="neutral" variant="ghost" size="xs" square :loading="isSaving(row.original)" @click="reopen(row.original)" /></template></div></template>
          </UTable>
        ...
        <section class="catalog-section" aria-labelledby="map-achievements-title">
          ...
          <UTable :data="mapItems" :columns="mapColumns" :loading="loading" empty="暂无记录。" class="admin-table achievement-table">
            ...
            <template #actions-cell="{ row }"><div class="table-actions"><template v-if="row.original.status !== 'retired'"><UPopover :open="planningId === row.original.challengeId" @update:open="(open) => { planningId = open ? row.original.challengeId : null; }"><UButton aria-label="计划下线" icon="i-lucide-calendar-clock" color="neutral" variant="ghost" size="xs" square :loading="isSaving(row.original)" /><template #content><UCard class="plan-popover-card"><form class="plan-popover" @submit.prevent="planSunsetting(row.original)"><UFormField label="计划下线版本" required><UInput v-model="retirementVersions[row.original.challengeId]" required placeholder="例如 26.0713.1" :disabled="isSaving(row.original)" /></UFormField><UButton type="submit" label="确认计划" :loading="isSaving(row.original)" :disabled="!retirementVersions[row.original.challengeId]?.trim()" /></form></UCard></template></UPopover><UButton aria-label="结束挑战" icon="i-lucide-circle-stop" color="error" variant="ghost" size="xs" square :loading="isSaving(row.original)" @click="openEnd(row.original, $event.currentTarget)" /></template><UButton v-else aria-label="重新开放" icon="i-lucide-rotate-ccw" color="neutral" variant="ghost" size="xs" square :loading="isSaving(row.original)" @click="reopen(row.original)" /></div></template>
          </UTable>
        </section>
      </div>
    </section>
...
.achievement-table [data-slot="th"]:last-child { width: 14%; }
...
.table-actions :deep(.portal-button) { flex: 0 0 auto; width: 34px; min-height: 34px; padding-inline: 0; }
