...
<template>
  <section class="batch-invites surface-card" aria-labelledby="batch-invites-title">
    <header class="batch-invites__header">
      <div>
        <p class="batch-invites__eyebrow">定向邀请</p>
        <h2 id="batch-invites-title">批量生成邀请码</h2>
      </div>
    ...
    <form class="batch-invites__form" @submit.prevent="createInvitations">
      <UTextarea
        v-model="input"
        :rows="7"
        aria-label="BattleTag 列表"
        placeholder="玩家名称#1234&#10;另一位玩家#5678"
      />
      <div class="batch-invites__actions">
        <p class="batch-invites__hint">每行一个 BattleTag</p>
        <UButton type="submit" label="生成邀请码" :loading="submitting" :disabled="!canSubmit" />
      </div>
    </form>
    ...
    <TransitionGroup v-if="invitations.length" name="invite-list" tag="div" class="invite-results" aria-label="已生成的邀请码">
      <article v-for="invitation in invitations" :key="invitation.inviteId" class="invite-result">
        <div class="invite-result__identity">
          <strong>{{ invitation.playerName }}#{{ invitation.playerId }}</strong>
          <code>{{ invitation.code }}</code>
        </div>
        <UButton
          :label="copiedInviteId === invitation.inviteId ? '已复制' : '复制口令'"
          :icon="copiedInviteId === invitation.inviteId ? 'i-lucide-check' : 'i-lucide-copy'"
          color="neutral"
          variant="outline"
          size="sm"
          :aria-label="`复制 ${invitation.playerName} 的绑定口令`"
...
<style scoped>
.batch-invites { display: grid; gap: 18px; padding: clamp(18px, 3vw, 28px); }
.batch-invites__header, .batch-invites__actions, .invite-result { display: flex; align-items: center; justify-content: space-between; gap: 16px; }
