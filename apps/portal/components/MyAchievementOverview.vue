      ...
      <section v-for="group in groups" :key="group.category" class="achievement-section" :aria-labelledby="`my-category-${group.category}`">
        ...
        <div class="achievement-grid">
          <article v-for="challenge in group.challenges" :key="challenge.challengeId" class="achievement-card" :class="{ earned: ownedTitleKeys.has(challenge.titleKey) }">
            ...
            <div class="achievement-copy"><strong>{{ challenge.titleName }}</strong><span>{{ challenge.condition }}</span><span v-if="ownedTitleKeys.has(challenge.titleKey)" class="earned-status-icon" role="img" aria-label="已获得"><UIcon name="i-lucide-circle-check" /></span><span v-else-if="challenge.status === 'scheduled'" class="status">未开放</span><span v-else-if="challenge.status === 'sunsetting'" class="status">即将结束</span></div>
          </article>
        </div>
      </section>
      ...
      <section v-if="historicalTitles.length" class="achievement-section historical-section" aria-labelledby="historical-title">
        ...
        <div class="achievement-grid">
          <article v-for="title in historicalTitles" :key="title.grantId" class="achievement-card earned">
            ...
            <div class="achievement-copy"><strong>{{ title.label }}</strong><span>{{ title.condition }}</span><span class="earned-status-icon" role="img" aria-label="已获得"><UIcon name="i-lucide-circle-check" /></span></div>
          </article>
        </div>
      </section>
