...
const historicalGlobalGroups = computed(() => groupHistoricalTitles(
  historicalTitles.value.filter((title) => title.scope === "global"),
  (title) => title.category,
));
const historicalMapGroups = computed(() => groupHistoricalTitles(
  historicalTitles.value.filter((title) => title.scope === "map"),
  (title) => title.mapName ?? "地图称号",
));
const historicalMapTitleCount = computed(() => historicalMapGroups.value.reduce((count, group) => count + group.titles.length, 0));
  ...
  <div class="achievement-layout">
    <div class="achievement-main">
      ...
      <section v-if="historicalTitles.length" class="achievement-section historical-section" aria-labelledby="historical-title">
        ...
        <section v-for="(group, index) in historicalGlobalGroups" :key="group.name" class="historical-group" :aria-labelledby="`historical-global-${index}`">
          <header class="historical-group-heading"><h3 :id="`historical-global-${index}`">{{ group.name }}</h3><span>{{ group.titles.length }} 项</span></header>
          <div class="achievement-grid">
        ...
        <section v-if="historicalMapGroups.length" class="historical-map-collection" aria-labelledby="historical-maps-title">
          <header class="historical-group-heading"><h3 id="historical-maps-title">地图称号</h3><span>{{ historicalMapTitleCount }} 项</span></header>
          <section v-for="(group, index) in historicalMapGroups" :key="group.name" class="historical-map" :aria-labelledby="`historical-map-${index}`">
            <header class="historical-map-heading"><h4 :id="`historical-map-${index}`">{{ group.name }}</h4><span>{{ group.titles.length }} 项</span></header>
            <div class="achievement-grid">
              <article v-for="title in group.titles" :key="title.grantId" class="achievement-card earned">
                <div class="achievement-icon" :class="{ 'has-image': title.iconUrl }" aria-hidden="true"><img v-if="title.iconUrl" :src="title.iconUrl" alt="" /><UIcon v-else :name="`i-lucide-${title.icon}`" /></div>
                <div class="achievement-copy"><strong>{{ title.label }}</strong><span>{{ title.condition }}</span></div>
